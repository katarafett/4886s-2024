...
double PID::get_adjustment(double setpoint, double sensor_value) {
...
void drive_straight(float inches, float target_ips, float ips_per_sec, bool do_decel) {
    ...
    while (ips >= 0 && std::abs(POS_DRIVE_L - pos_start_l) < std::abs(inches)) {
        ...
        // Maintain speed
        pid_adjustment_l = pid_drive_l.get_adjustment(pos, pos_l);
        pid_adjustment_r = pid_drive_r.get_adjustment(pos, pos_r);
        pid_adjustment_dir = pid_dir.get_adjustment(target_heading, ROTATION * GYRO_CORRECTION);
...
void drive_turn(float degrees, float outer_radius, float target_ips, float ips_per_sec, bool reversed) {
    ...
    while (ips >= 0) {
        ...
        if ((reversed && degrees > 0) || (!reversed && !(degrees > 0))) { // left is inner side
            pid_adjustment_l = -1 * pid_drive_l.get_adjustment(inner_pos, pos_l);
            pid_adjustment_r = pid_drive_r.get_adjustment(outer_pos, pos_r);
            ...
            pid_adjustment_l = pid_drive_l.get_adjustment(outer_pos, pos_l);
            pid_adjustment_r = -1 * pid_drive_r.get_adjustment(inner_pos, pos_r);
...
void drive_linear(float inches, float max_ips, float ips_per_sec, bool do_decel) {
    ...
    while (std::abs(avg_pos) < std::abs(inches)) { // cuts if we've reached target position
        ...
        pid_l_adj = pid_l.get_adjustment(target_vel_l, VEL_DRIVE_L);
        pid_dir_adj = pid_dir.get_adjustment(target_heading, ROTATION * GYRO_CORRECTION);
...
void turn_pid(float degrees, float ratio, int direction) {
    ...
    while (time_still < 80) {
        ...
        speed_l = drive_pid.get_adjustment(target_heading, ROTATION * GYRO_CORRECTION) * direction;
...
void straight_pid(float dist) {
    ...
    while (time_still < 80) {
        speed_l = drive_pid.get_adjustment(dist, POS_DRIVE_L - start_pos_l);
        speed_r = drive_pid.get_adjustment(dist, POS_DRIVE_R - start_pos_r);
        dir_adjustment = dir_pid.get_adjustment(target_heading, ROTATION * GYRO_CORRECTION);
